...
public class AppController extends Application {
    ...
    public static final long LOCATION_UPDATER_DELAY = 1 * 60 * 60 * 1000; // time in milli seconds (default 1 hour)
    public static final int MAX_LOCATION_UPDATER_TRIES = 5;
    public static final String CURRENCY = "L.E.";
    ...
    public void onCreate() {
        ...
        String response = getActiveUserResponse(getApplicationContext());
        if (response != null) {
            // handle it in user object
            UserHandler userHandler = new UserHandler(response);
            // save current user in global app controller
            getInstance(getApplicationContext()).activeUser = userHandler.handle();
        }
    }
}
